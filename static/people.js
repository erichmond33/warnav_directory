...
function applySorting(sortBy) {
    ...
    // Get the Warnav community card
    const warnavCard = document.getElementById('warnav-community-card');
    ...
    // Re-append the sorted cards to the container, ensuring Warnav card stays in 4th position
    sortedCards.forEach((card, index) => {
        if (index === 3 && warnavCard) {
            // Insert Warnav card at 4th position
            peopleContainer.appendChild(warnavCard);
        }
        peopleContainer.appendChild(card);
    });
    
    // If there are fewer than 3 cards and we have a Warnav card, append it at the end
    if (sortedCards.length < 3 && warnavCard) {
        peopleContainer.appendChild(warnavCard);
    }
}
